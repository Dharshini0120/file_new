...
const TemplateQuestionNode: React.FC<QuestionNodeProps> = ({ data, id }) => {
    ...
    return (
        <>
            ...
            {showEditModal && (() => {
                // Use optionsData if available, otherwise reconstruct from options array
                let finalOptions;
                if (optionsData && Array.isArray(optionsData) && optionsData.length > 0) {
                    console.log('🎯 Using existing optionsData:', optionsData);
                ...
                const questionData = {
                    ...
                    isRequired,
                    optionsData: finalOptions // Pass the converted options as optionsData
                };
